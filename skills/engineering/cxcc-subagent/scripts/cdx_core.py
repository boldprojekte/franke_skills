...
def launch_helper(root: Path, name: str, prompt_path: Path, mode: str, stall_after: int, hard_kill_after: int, backend: str, backend_bin: str) -> int:
    cmd = [
        sys.executable,
        str(Path(__file__).with_name("cdx.py").resolve()),
        "__run_turn",
        "--state-dir",
        str(root),
        "--task",
...
def command_hint(args: argparse.Namespace, command: str) -> str:
    parts = [shlex.quote(sys.executable), shlex.quote(str(Path(__file__).with_name("cdx.py").resolve()))]
...
def home(args: argparse.Namespace) -> int:
    ...
    binary = str(Path(__file__).with_name("cdx.py").resolve())
